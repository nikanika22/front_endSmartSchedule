...
const RegisterPage: React.FC<RegisterPageProps> = ({ isAdminMode = false }) => {
  ...
  return (
    <div className={isAdminMode ? "flex justify-center w-full" : ""}>
      <CardCustom className={isAdminMode ? "w-full max-w-[600px] mt-8 bg-white rounded-3xl p-6 sm:p-8 shadow-sm" : "w-full max-w-[420px] mx-auto border border-white/60 !bg-white/80 backdrop-blur-xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-3xl p-4 sm:p-6"}>
